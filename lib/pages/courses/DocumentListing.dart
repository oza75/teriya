...
class DocumentListing extends StatefulWidget {
  final Future<List<CourseDocument>> future;
  final Function() onFetchDocuments;
  final String title;
  final Future Function(List<File>) onUpload;

  const DocumentListing({
    super.key,
    required this.future,
    required this.onFetchDocuments,
...
class _DocumentListingState extends State<DocumentListing> {
  ...
  void _showAddDocumentModal(context) async {
    ...
    if (result != null) {
      ...
      widget.onUpload(files).then((res) {
        showSnackbar(
          context,
          Text(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              return documents.length == 0
                  ...
                  : DocumentItemsList(
                      documents: documents,
                      onRemoved: (doc) => widget.onFetchDocuments(),
                    );
            }
          },
        ),
...
class DocumentItemsList extends StatefulWidget {
  final List<CourseDocument> documents;
  final Function(CourseDocument) onRemoved;
  ...
  @override
  State<DocumentItemsList> createState() => _DocumentItemsListState();
}

class _DocumentItemsListState extends State<DocumentItemsList> {
